...
export default function Admin() {
  ...
  const handleItemChange = (categoryKey, itemIndex, field, value) => {
    setMenuData(prev => {
      const newData = { ...prev };
      newData[categoryKey] = { ...newData[categoryKey] };
      newData[categoryKey].items = [...newData[categoryKey].items];
      newData[categoryKey].items[itemIndex] = { ...newData[categoryKey].items[itemIndex], [field]: value };
      return newData;
    });
  };
  ...
  const handleCategoryNameChange = (categoryKey, newLabel) => {
    setMenuData(prev => {
      const newData = { ...prev };
      newData[categoryKey] = { ...newData[categoryKey], label: newLabel };
      return newData;
    });
  };
  ...
  const handleAddItem = (categoryKey) => {
    setMenuData(prev => {
      const newData = { ...prev };
      newData[categoryKey] = { ...newData[categoryKey] };
      newData[categoryKey].items = [...newData[categoryKey].items, { name: '', desc: '', price: '', tag: '' }];
      return newData;
    });
  };
  ...
  const handleDeleteItem = (categoryKey, itemIndex) => {
    if (window.confirm('Gericht wirklich löschen?')) {
      setMenuData(prev => {
        const newData = { ...prev };
        newData[categoryKey] = { ...newData[categoryKey] };
        newData[categoryKey].items = newData[categoryKey].items.filter((_, i) => i !== itemIndex);
        return newData;
      });
    }
  };
  ...
  return (
    <div className="admin-dashboard">
      ...
      {isLoading || !menuData ? (
        <p style={{ textAlign: 'center', color: 'var(--color-text-secondary)' }}>Lade Speisekarte...</p>
      ) : (
